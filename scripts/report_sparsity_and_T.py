...
# --- ▼ 修正(v3): DataLoader, TensorDataset をインポート ▼ ---
# --- ▼ 修正(v4): TensorDataset は不要になったため削除 ▼ ---
...
# ... existing code ...
    ...
    # --- ▼ 修正 (v3): cfg は既に DictConfig オブジェクト ▼ ---
...
# ... existing code ...
    collate_fn_factory: Any = container.collate_fn_factory() # 実際には text_collate_fn
...
# ... existing code ...
    ...
    # (注: BreakthroughTrainer の __init__ は config を受け取らないが、
    #  DIコンテナが config.training.gradient_based... を読み取るために cfg が必要)
...
# ... existing code ...
        ...
        # config=cfg # standard_trainer は config を引数に取らない
        # standard_trainer のファクトリが cfg を参照するので、
        # container.config.from_dict(cfg_dict) が必要だった
    )
    # --- ▲ 修正 (v4) ▲ ---
