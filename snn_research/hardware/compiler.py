...
#
# 修正 (v19):
# - BioLIFNeuronのパラメータ抽出で、適応的閾値のパラメータが正しく含まれるように修正。
# - BioSNNの属性アクセスに対して明示的な型キャストを行い、mypyエラーを解消。
...
class NeuromorphicCompiler:
    ...
    def _get_neuron_type_and_params(self, module: nn.Module) -> Tuple[str, Dict[str, Any]]:
        ...
        if isinstance(module, AdaptiveLIFNeuron):
            ...
            params = { "a": getattr(module, 'a', 0.02), "b": getattr(module, 'b', 0.2), "c": getattr(module, 'c', -65.0), "d": getattr(module, 'd', 8.0), "dt": getattr(module, 'dt', 0.5) }
             ...
             params = {
                 ...
                 "v_threshold": getattr(module, 'v_thresh_base', getattr(module, 'v_thresh', 1.0)), # P8.3互換性 (v_thresh_base or v_thresh)
        ...
        for k, v in params.items():
            if isinstance(v, torch.Tensor):
                serializable_params[k] = v.mean().item() if v.numel() > 0 else v.tolist() # スカラーテンソル対応
    ...
    def _analyze_model_structure(self, model: nn.Module) -> Dict[str, Any]:
        ...
        if isinstance(model, BioSNN):
            model_bio = cast(BioSNN, model)
            first_conn_input_size = model_bio.input_size 
        else:
        ...
        if isinstance(model, BioSNN):
             model_bio = cast(BioSNN, model)
             last_conn_output_size = model_bio.layer_configs[-1]["n_e"] # Eニューロン数
        else:
        ...
        if isinstance(model, BioSNN):
            model_bio = cast(BioSNN, model)
            model_bio.clamp_weights() # デール則適用後の重みで解析
            
            current_n_e = model_bio.input_size
            current_n_i = 0
            pre_e_name = "input"
            pre_i_name: Optional[str] = None
            
            for i in range(len(model_bio.layer_configs)):
                post_e_name = f"layers_e.{i}"
                post_i_name: Optional[str] = f"layers_i.{i}" if i < len(model_bio.layers_i) else None
                
                n_e_post = model_bio.layer_configs[i]["n_e"]
                n_i_post = model_bio.layer_configs[i].get("n_i", 0)

                # E -> E
                w_ee = model_bio.weights_ee[i]
                num_conn_ee = (w_ee.data > 1e-9).sum().item() # 0でない接続
                structure["connections"].append({
                    ...
                    "num_synapses": num_conn_ee
                })
                connection_count += num_conn_ee

                if n_i_post > 0 and i < len(model_bio.weights_ie):
                    # E -> I
                    w_ie = model_bio.weights_ie[i]
                    num_conn_ie = (w_ie.data > 1e-9).sum().item()
                    structure["connections"].append({
                        ...
                        "num_synapses": num_conn_ie
                    })
                    connection_count += num_conn_ie

                if current_n_i > 0 and pre_i_name is not None:
                    if i-1 < len(model_bio.weights_ei):
                        # I -> E (inhibitory component is implicit in the connection, weights are stored as positive)
                        w_ei = model_bio.weights_ei[i-1]
                        num_conn_ei = (w_ei.data > 1e-9).sum().item()
                        structure["connections"].append({
                            "source_module": pre_i_name, "target_module": post_e_name,
                            "connection_module_name": f"weights_ei.{i-1}", "type": "dense_ie", # I->E is inhibitory
                            "in_features": current_n_i, "out_features": n_e_post,
                            "num_synapses": num_conn_ei 
                        })
                        connection_count += num_conn_ei
                    
                    if n_i_post > 0 and i-1 < len(model_bio.weights_ii):
                        # I -> I
                        w_ii = model_bio.weights_ii[i-1]
                        num_conn_ii = (w_ii.data > 1e-9).sum().item()
                        structure["connections"].append({
                            ...
                            "num_synapses": num_conn_ii 
                        })
                        connection_count += num_conn_ii
    ...
    def _generate_hardware_config(self, model: nn.Module, target_hardware: str) -> dict:
        ...
        if isinstance(model, BioSNN):
            model_bio = cast(BioSNN, model)
            if hasattr(model_bio, 'synaptic_rule') and isinstance(model_bio.synaptic_rule, BioLearningRule):
                rule: BioLearningRule = model_bio.synaptic_rule
