...
#
# --- 修正 (mypy) ---
# 1. [no-redef] エラー解消のため、logger を
#    インポート直後に1回だけ定義するよう修正。
...
# --- ▼ 修正 (mypy): logger をモジュールトップに1回だけ定義 ▼ ---
...
try:
    ...
    # logger = logging.getLogger(__name__) # 削除
...
# logger: logging.Logger = logging.getLogger(__name__) # 削除
...
class AdaptiveLIFNeuron(BaseModel): 
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        """
        if not self.built:
            # (mypy互換性) self.name が BaseModel に存在しない可能性があるため修正
            layer_name = getattr(self, 'name', 'This layer')
            raise RuntimeError(f"Layer {layer_name} has not been built.")
        ...
        # スパイクカウント（BaseModel用）
        # (base.py の __init__ で total_spikes が初期化される前提)
        if hasattr(self, 'total_spikes'):
            # --- 修正 (mypy): [has-type] エラー回避のため型を明示 ---
            self.total_spikes = cast(Tensor, self.total_spikes) + torch.sum(spikes_t)
